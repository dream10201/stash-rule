...
whois -h whois.radb.net -- '-i origin AS32934' | perl -ne 'print "$1\n" if /^route.+\s+(.+)/'>>ipcidr-text.txt
whois -h whois.radb.net -- '-i origin AS13414' | perl -ne 'print "$1\n" if /^route.+\s+(.+)/'>>ipcidr-text.txt
whois -h whois.radb.net -- '-i origin AS4694' | perl -ne 'print "$1\n" if /^route.+\s+(.+)/'>>ipcidr-text.txt
